...
function sendToServer(x, y, r) {
    const url = `/fcgi-bin/check?x=${encodeURIComponent(x)}&y=${encodeURIComponent(y)}&r=${encodeURIComponent(r)}`;
...
function formatLongY(y) {
    const str = y.toString();
    if (str.includes('.')) {
        const [int, frac] = str.split('.');
        return int + '.' + Array.from(frac).join('<wbr>');
    }
    return str;
}
